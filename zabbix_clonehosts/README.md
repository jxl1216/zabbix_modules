...
1. **下载压缩包**
   ...
   前往 [Releases 页面](https://github.com/jxl1216/zabbix_modules/releases)，下载最新版本的 `zabbix_clonehosts-x.x.x.tar.gz` 文件到本地。如下载 `zabbix_clonehosts-1.2.tar.gz`
...
本模块通过 GitHub Actions 自动打包发布。当 `manifest.json` 中的 `version` 字段升级并推送到主分支时，将自动触发打包流程，生成 `zabbix_clonehosts-<version>.tar.gz` 压缩包并发布到 [Releases 页面](https://github.com/jxl1216/zabbix_modules/releases)。
